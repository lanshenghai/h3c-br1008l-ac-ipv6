# 已修复固件

| 文件 | 说明 |
|------|------|
| `HM1A0V100R015.bin` | **推荐**刷入（当前；全量作废旧前缀 RA + 15s 监视） |
| `HM1A0V100R014.bin` | 历史：前缀监视 + 单旧前缀作废 RA（约 3s） |
| `HM1A0V100R013.bin` | 历史：事件钩子 + `expr` |
| `HM1A0V100R012.bin` | 历史：Option A + `etc/rc` 晚启动钩子 |

各文件为**独立镜像**（版本头与文件名一致）。构建时只更新当前版本，不会用新包覆盖旧文件名。

基线：官方 `HM1A0V100R011`。版本演进详见仓库首页 [README · 固件版本差异](../README.md#固件版本差异)。

## 版本差异（摘要）

| 版本 | 相对上一版 | 要点 |
|------|------------|------|
| **R011** | 官方原厂 | 无补丁 |
| **R012** | ← R011 | Option A + `etc/rc` 晚启动钩子 |
| **R013** | ← R012 | 事件钩子 + `expr`；GUI 开关可恢复 |
| **R014** | ← R013 | 前缀更换：按路由 `src` 选 `/64` + 后台监视；作废**上一个**旧前缀 |
| **R015** | ← R014 | 全量作废旧前缀（30s）+ 15s 监视；空 pdconf 不触发 dep_all；启动等 RA 5s |

## 补丁内容（仅 AC 模式，R015）

| 项 | 说明 |
|----|------|
| Option A | `ipv6rahandle`：AC → 在 `lan1` 收 RA；路由器/PPPoE 不变 |
| 事件钩子 | 包装 `/bin/ipv6rahandle`：Web/开机启用 IPv6 时触发 |
| `accept_ra=2` | 转发口可学上游 RA |
| `/var/dhcp6pd.conf` + `cm dhcp6c_get` | 官方 mode=1 → `radvd` 广播真实 Prefix |
| 前缀监视 | 默认路由 `src` 变化时刷新 pdconf/`radvd` |
| 全量作废 RA | `dep_all`：对所有已知旧前缀 Preferred=0，广播约 30s |
| ash 兼容 | arm 用 `expr`，不用 `$((arith))` |

`productmodeflag≠1`（路由器/PPPoE）时 arm 直接退出。

升级与验证见仓库根目录 [README.md](../README.md)。
