...
- [`firmware/HM1A0V100R013.bin`](firmware/HM1A0V100R013.bin)（推荐）  
- [`firmware/HM1A0V100R012.bin`](firmware/HM1A0V100R012.bin)（与 R013 **同一镜像**，仅文件名不同，便于覆盖旧包）

基于官方 `HM1A0V100R011` 改包，版本头为 `HM1A0V100R013`，并已按设备校验方式重签 payload checksum。
...
2. **事件钩子**：包装 `/bin/ipv6rahandle`，每次 **启用 IPv6**（开机或 Web）时自动：  
   - `accept_ra=2`  
   - 等 `lan1` 出现全局 `/64` 后写 `/var/dhcp6pd.conf`  
   - `cm dhcp6c_get` 刷新 `radvd`  
3. arm 脚本只用 `expr`（设备 ash **不支持** `$((…))`），避免钩子启动即语法失败  

**不是**后台定时轮询；也 **不需要** 软路由侧热补丁 / watchdog。
...
**仓库文件名说明：** 当前 [`HM1A0V100R012.bin`](firmware/HM1A0V100R012.bin) 与 [`HM1A0V100R013.bin`](firmware/HM1A0V100R013.bin) **内容相同**（均为 R013 镜像），只是多留一个旧文件名方便覆盖升级。设备上真正的版本字符串是 **R013**。若你以前刷过「历史 R012」（`etc/rc` 钩子那版），请再刷一次本仓库的 R013。

### 使用步骤

1. **GUI 启用 IPv6**（见上文「前提」）  
2. 浏览器打开 AC 管理页 → **本地升级 / 固件升级**  
3. 选择 `firmware/HM1A0V100R013.bin`，等待升级完成并重启  
